  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:py-12">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900">¿En qué te ayudo hoy?</h2>
            ...
            <div className="space-y-1.5">
              {cat.preguntas.map(p => (
                <button
                  ...
                  className="block w-full rounded-lg px-2.5 py-2 text-left text-sm text-gray-600 transition-colors hover:bg-primary/5 hover:text-primary"
                >
                  {p}
                </button>
              ))}
        ...
        {mensajes.map((m, i) => {
          ...
          return (
            <div key={m.id ?? i} className={`flex gap-3 ${esAsistente ? 'flex-row' : 'flex-row-reverse'}`}>
              <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${esAsistente ? 'bg-primary' : 'bg-gray-200'}`}>
                {esAsistente ? <Bot className="h-4 w-4 text-primary-foreground" /> : <User className="h-4 w-4 text-gray-600" />}
              </div>
              <div className={`min-w-0 max-w-[85%] rounded-2xl px-4 py-3 ${
                ...
                {esAudio && !esAsistente && (
                  <span className="mb-1 flex items-center gap-1 text-[11px] text-primary-foreground/70">
