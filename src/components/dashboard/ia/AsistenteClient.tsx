...
export function AsistenteClient({ userId, carpetasIniciales, conversacionesIniciales, personas, promptInicial }: Props) {
  // El cliente tipado de este proyecto devuelve `never` en escrituras; se usa un
  // alias laxo para CRUD (mismo patrón que otros módulos del repo).
  ...
  // ── Persistencia (tolerante a fallos: el chat sigue en memoria) ──
  ...
  const enviar = useCallback(async (texto: string, fromAudio = false) => {
    ...
    try {
      const res = await fetch('/api/ia/chat', {
        ...
        body: JSON.stringify({
          mensajes: historialParaApi,
          modelo,
          persona: personaCtx ? { id: personaCtx.id, tipo: personaCtx.tipo } : null,
        }),
      ...
      for (;;) {
        ...
        setMensajes(prev => {
          ...
          if (last) copia[copia.length - 1] = { ...last, content: acumulado }
          return copia
        })
      }
  ...
  return (
    <div className="flex h-full overflow-hidden bg-white">
      {/* Sidebar desktop */}
      <aside className="hidden w-72 shrink-0 border-r border-gray-200 lg:block">{sidebar}</aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center gap-2 border-b border-gray-200 bg-white px-3 py-2.5 sm:px-4">
          <button onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 hover:bg-gray-100 lg:hidden" aria-label="Historial">
            <Menu className="h-5 w-5 text-gray-600" />
          </button>

          <div className="flex min-w-0 items-center gap-2">
            <span className="hidden h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 sm:flex">
              <Sparkles className="h-4 w-4 text-primary" />
            </span>
            <div className="min-w-0">
              <h1 className="truncate text-sm font-bold text-gray-900">{convActiva?.titulo ?? 'Asistente IA'}</h1>
              <p className="text-[11px] text-gray-400">Plataforma UTL 360 · Jairo León Vargas</p>
            </div>
          </div>
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-primary px-2.5 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90"
            >
        ...
        {/* Contexto de persona */}
        <div className="border-t border-gray-100 bg-white px-3 pt-2 sm:px-4">
          <div className="mx-auto flex max-w-3xl items-center gap-2">
            {personaCtx ? (
              <span className="flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-xs text-primary">
                <User className="h-3.5 w-3.5" />
                <span className="font-medium">{personaCtx.nombres} {personaCtx.apellidos}</span>
                <span className="text-primary/60">· {personaCtx.tipo === 'ciudadano' ? personaCtx.documento : 'contacto'}</span>
                <button onClick={() => setPersonaCtx(null)} className="ml-0.5 rounded-full p-0.5 hover:bg-primary/20" aria-label="Quitar persona">
                  <X className="h-3 w-3" />
                </button>
              </span>
            ) : (
                    ...
                    <div className="absolute bottom-9 left-0 z-40 w-72 rounded-xl border border-gray-200 bg-white shadow-lg">
                      ...
                      <div className="max-h-56 overflow-y-auto py-1">
                        ...
                        {personasFiltradas.map((p) => (
                          <button key={`${p.tipo}-${p.id}`} onClick={() => { setPersonaCtx(p); setPersonaOpen(false); setPersonaQuery('') }}
                            ...
                            <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-medium',
                              p.tipo === 'ciudadano' ? 'bg-primary/10 text-primary' : 'bg-amber-100 text-amber-700')}>
                              {p.tipo}
                            </span>
                          </button>
                        ))}
